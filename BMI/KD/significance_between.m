function [p1, c1] = significance_between(inputs, groups, test, ctype)
p1=[]; c1 = [];    
    for j=1:size(inputs,2)
        ...
        for i = 1:size(inputs,1)
            ...
            data = vertcat(data, inputs{i,j}); 
            name = groups(i);
            r = repmat(name, length(inputs{i,j}),1); 
            origin = vertcat(origin, r); 
        end

        %% ANOVA for mean frequencies
        switch test
            case 'annova1'
                [p,~,stats] = anova1(data,origin,'off'); 
            case 'kw'
                [p,~,stats] = kruskalwallis(data,origin,'off'); 
        end
        p1(j) = p; 
         [c,~,~,~]  = multcompare(stats,'CType', ctype,'Display','off');
            [c,~,~,~]  = multcompare(stats,'CType','tukey-kramer', 'CType','dunn-sidak', 'CType','bonferroni','CType', 'scheffe','Display','off'); 
        c1{j} = c; 
        k=1; clear pairs pvalues
        for i=1:size(c,1)
            if c(i,6) <= 0.05
                if j==1
                pairs{k} = c(i,1:2)-0.25; 
                elseif j==2
                    pairs{k} = c(i,1:2); 
                elseif j==3
                    pairs{k} = c(i,1:2)+0.25; 
...
function [p1, c1] = significance_within(inputs, boxes, positions, test, ctype)
p1=[]; c1 = [];    
    for j=1:size(inputs,1)
        ...
        for i = 1:size(inputs,2)
            ...
            data = vertcat(data, inputs{j,i}); 
            name = boxes(i);
            r = repmat(name, length(inputs{j,i}),1); 
            origin = vertcat(origin, r); 
        end
...
%             [c,~,~,~]  = multcompare(stats,'CType', ctype,'Display','off');
            ...
            for i=1:size(c,1)
                if c(i,6) <= 0.05
    %                 pairs{k} = j-1+(0.25*c(i,1:2)+0.5); 
                    pairs{k} = positions(((j-1)*length(boxes))+c(i,1:2)); 
                    pvalues(k) = c(i,6);
                    k=k+1;
                end
            end
            if exist('pairs')
                clear a idx pairs_sorted
                for ind = 1:length(pairs)
                    a(ind) = diff(pairs{ind});                     
                end
                [~, idx] = sort(a); 
                for ind=1:length(pairs)
                    pairs_sorted{ind} = pairs{idx(ind)}; 
                end
                sigstar(pairs_sorted, pvalues,1)
            end
        end
    end
...
function varargout=sigstar(groups,stats,nosort,color)
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii),color);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,c)
    ...
    H(1)=plot(x(:),y,c,'LineWidth',2,'Tag','sigstar_bar');
