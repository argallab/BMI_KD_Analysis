...
bag_folder = '/home/tem/Documents/MATLAB/BMI/KD/6-28-19_Tem/Task3/'; %path to appropriate folder containing the .bag files. 
fnames = dir(bag_folder);
numfids = length(fnames);

source_frame = 'j2s7s300_link_base';
target_frame = 'j2s7s300_end_effector';
jaco_task3_translation = []; %matrix for storing translation data
jaco_task3_rotation = []; %matrix for storing quaternion data
for i=3:numfids
    ...
    jaco_task3_translation = [jaco_task3_translation; jaco_translation];
    %append orientation data from each trial
    jaco_task3_rotation = [jaco_task3_rotation; jaco_rotation];
end

%%

save('/home/tem/Documents/MATLAB/BMI/KD/6-28-19_Tem/MAT_Data/jaco_task3_translation.mat', 'jaco_task3_translation');
save('/home/tem/Documents/MATLAB/BMI/KD/6-28-19_Tem/MAT_Data/jaco_task3_rotation.mat', 'jaco_task3_rotation');
...
offset = 0.2;
xmin = min(jaco_task3_translation(:, 1)); xmax = max(jaco_task3_translation(:, 1)); xrange = [xmin-offset, xmax+offset];
ymin = min(jaco_task3_translation(:, 2)); ymax = max(jaco_task3_translation(:, 2)); yrange = [ymin-offset, ymax+offset];
zmin = min(jaco_task3_translation(:, 3)); zmax = max(jaco_task3_translation(:, 3)); zrange = [zmin-offset, zmax+offset];
offset = 0.3;
...
% Scatterplot
s1 = scatter3(jaco_task3_translation(:, 1), jaco_task3_translation(:, 2), jaco_task3_translation(:, 3), 'k')
...
% PCA
[coeff, score, latent, tsquared, explained, mu] = pca(jaco_task3_translation);
...
offset = 0.2;
xmin = min(jaco_task3_rotation(:, 1)); xmax = max(jaco_task3_rotation(:, 1)); xrange = [xmin-offset, xmax+offset];
ymin = min(jaco_task3_rotation(:, 2)); ymax = max(jaco_task3_rotation(:, 2)); yrange = [ymin-offset, ymax+offset];
zmin = min(jaco_task3_rotation(:, 3)); zmax = max(jaco_task3_rotation(:, 3)); zrange = [zmin-offset, zmax+offset];
offset = 0.3;
xrange = [-0.8, 0.8]; yrange = [-0.8, 0.8]; zrange = [-0.8, 0.8];
...
% Scatterplot
s1 = scatter3(jaco_task3_rotation(:, 1), jaco_task3_rotation(:, 2), jaco_task3_rotation(:, 3), 'k')
xlabel('X'); ylabel('Y'); zlabel('Z');
axis([xrange, yrange, zrange]);
axis square;
view([142,31]);
...
% PCA
[coeff, score, latent, tsquared, explained, mu] = pca(jaco_task3_rotation);
